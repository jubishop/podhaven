...
@MainActor
struct SearchPickIndex {
  private struct Key: Hashable {
    let feedURL: FeedURL
    let mediaGUID: MediaGUID
  }

  private var entriesByPick: [Key: SearchCachedPodcastEntry] = [:]
  ...
  mutating func register(
    ...
    for entry: SearchCachedPodcastEntry
  ) {
    unregister(of: entry)
    for pick in scored {
  ...
  mutating func unregister(of entry: SearchCachedPodcastEntry) {
  ...
  mutating func remove(mediaGUID: MediaGUID, feedURL: FeedURL) -> SearchCachedPodcastEntry? {
