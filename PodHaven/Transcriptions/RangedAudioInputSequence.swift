...
enum SpeechAnalyzerInputError: LocalizedError {
...
struct RangedAudioInputSequence: AsyncSequence, Sendable {
  typealias Element = RangedAudioInput

  struct AsyncIterator: AsyncIteratorProtocol {
    let state: State

    mutating func next() async throws -> RangedAudioInput? {
  ...
  actor State {
    ...
    private let file: any AudioFileReading
    ...
    func next() throws -> RangedAudioInput? {
...
// Each buffer leaves actor-owned storage before crossing isolation and is never
// mutated afterward; AVAudioPCMBuffer itself does not declare Sendable.
struct RangedAudioInput: @unchecked Sendable {
