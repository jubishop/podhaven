...
- To load the real device DB into the Simulator: export it on the phone via Settings → Debug → "Share PodHaven Database", then drop it into the dev build's `PodHavenDev/` directory. The path differs (production uses root `Documents`, dev uses the subdir) but the SQLite file itself is interchangeable. Delete any stale `db.sqlite-wal` / `db.sqlite-shm` after swapping — the app recreates them.
