...
description: Treat the scheduled memory audit as semantic curation and keep DeepSeek V4 Flash until task-specific evidence supports a sub-$0.50 replacement.
type: feedback
---

# Memory-audit model selection

Evaluate the scheduled memory audit as retrieval-augmented semantic curation, not as coding-agent work. Keep `deepseek/deepseek-v4-flash` as the default unless a task-specific comparison demonstrates a meaningful quality improvement while staying at or below $0.50 per completed audit.
...
Prefer the current model unless the candidate produces a clear quality gain within the $0.50 budget. Do not switch based only on a generic leaderboard or model branding. For an exact cost comparison, retain the input, cached-input, reasoning, and output token breakdown; the published audit artifacts currently retain only total cost.
