...
**How to apply:** When fixing performance or fan-out issues in `PodcastRecommendationScorer` (the podcast-detail scorer extracted out of `PodcastDetailViewModel`), `EpisodesListViewModel`, or `RecommendationEngine.contextRevision` handling, keep bounded/coalesced prewarming. Do not simply gate all recommendation-score computation behind the recommendation-score sort being selected; apply UI sorting/filtering only when selected, but allow background score computation/cache refresh ahead of selection.
