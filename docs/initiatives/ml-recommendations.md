...
### Foreground embedding observation — shipped 2026-05-17

The BG task remained the only path to compute embeddings, so newly-fetched episodes waited up to an iOS BG window (often >1 hour) before recommendations and similarity scores covered them. `EmbeddingProcessor` now also runs a long-running observation while the app is active: on `.active` it spawns a `.background`-priority Task that awaits asset readiness, subscribes to `Observatory.episodesNeedingEmbeddings(revision:)`, and drains the list through `EmbeddingService.upsertEpisodeEmbeddings(forIDs:embedding:)`. On `.background` the task is cancelled and `BackgroundTaskScheduler.scheduleNext()` takes over. `upsertEpisodeEmbeddings` already checks `Task.checkCancellation()` per episode and flushes pending writes on cancel, so the handoff preserves in-flight work. No coordination with the pending BG submission is needed — by the time it fires, the queue is empty (or nearly so) and the handler returns quickly.
...
Files: `PodHaven/Utility/AsyncLatch.swift` (new), `PodHaven/Recommendations/Embeddings/EmbeddingProcessor.swift`, `PodHaven/Recommendations/Embeddings/ContextualEmbedding.swift`, `PodHaven/Database/Observatory.swift`, `PodHaven/Database/RecommendationRepo.swift`, `PodHaven/Utility/DownloadManager.swift`. Tests: `AsyncLatchTests`, `EmbeddingProcessorTests` (active drains backlog, background cancels, repeated active is idempotent, retries after observation failure), and the `assetsLoaded` waiter in `ContextualEmbeddingTests`.
