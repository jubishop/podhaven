...
- The observation closure inside `Observatory.scoringContextInputs()` builds a `TrackedScoringSources` directly from rated signals, signal embeddings, the embedding-row count, and freshness cadences. It intentionally omits the `PartialSignal` fetch — that keeps `playbackCoverage` and `lastPlayedDate` out of the tracked region so per-tick `updatePlayback` writes do not wake the observation. `PartialSignal` is fetched only inside `Repo.latestScoringContextInputs()`, which the engine calls on rebuild.
