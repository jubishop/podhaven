...
- `RecommendationEngine` with dual positive/negative centroids, scoring blend (similarity 0.80, podcast affinity 0.20) plus a multiplicative freshness gate, Bayesian smoothing, temporal decay, missing-feature renormalization, confidence floor.
...
- `lastPlayedDate`/`playbackCoverage` NOT part of OnDeck decoding (avoids observation churn).
...
- **Scoring weights (as of 2026-04-27):** similarity 0.80, podcast affinity 0.20. Freshness is no longer a summand; it is a cadence-derived multiplicative gate. Tag affinity and duration fit were removed (see deferred section).

## Rating-UI callsite contract (UI-branch work)

After `repo.updateRating(episodeID, rating:)`, the UI callsite must kick off a non-blocking `Task` that calls `EmbeddingService.upsertEpisodeEmbeddings(for: [episode], embedding: Container.shared.contextualEmbedding())` for the just-rated episode. This ensures the next recommendation generation has the rated episode's vector ready without waiting for the general embedding drain. `Repo` stays pure — embedding is NOT called inside `updateRating` itself.
...
**Schema (migration v41).** Two new columns on `episode`:
...
- Both call sites — the observation in `Observatory.scoringContextInputsWithoutPartialSignals()` and the engine's debounced rebuild in `Repo.allScoringContextInputs()` — share a single static builder, `Repo.scoringContextInputs(_:partialSignals:)`. The observation passes the default empty closure, so `partialSignals` is always `[]` and `playbackCoverage` / `lastPlayedDate` stay out of GRDB's tracked region; per-tick `updatePlayback` writes therefore do not wake the observation. The rebuild path passes a real `PartialSignal` fetcher.
- `RecommendationEngine` subscribes to `SharedState.$onDeck` and treats every transition (including nil → new) as a session boundary — that's the trigger that catches partial-signal staleness within a playback session. Both triggers (GRDB observation and onDeck transition) feed a single debounced rebuild that always re-reads `Repo.allScoringContextInputs()`, so whichever wins the debounce sees fresh DB state at fire time.
...
**Files touched:**
- New: `Recommendations/Models/PlaybackCoverage.swift`, `Recommendations/Models/PartialSignal.swift`, migration `v41`, tests `PlaybackCoverageTests`, `PlaybackCoverageRepoTests`, `V41MigrationTests`.
- Modified: `Recommendations/Models/SignalEpisode.swift` (rated-only), `Recommendations/Models/ScoringContextInputs.swift` (two lists), `Recommendations/RecommendationEngine.swift` (weights + two-loop centroid), `Database/Models/Episode.swift` (`hasSignal` SQL + `hasCoverage` + new column refs), `Database/Observatory.swift` (`scoringContextInputsWithoutPartialSignals()`), `Database/Repo.swift` (`updatePlayback`, `allUnratedListenedEpisodes`, `allScoringContextInputs()`), `Play/Utility/PodAVPlayer.swift` (`savePlaybackTick`).

## Deferred to v2

### Additional freshness tuning

Per-podcast freshness is now modeled as `FreshnessCadence`: `nil` means auto-infer from recent pubDates, and manual choices are hourly, twiceDaily, daily, twiceWeekly, weekly, monthly, and evergreen. Inference thresholds (`MaxMedianHours`) and the decay half-life (`halfLifeHours`) are expressed in hours so the sub-daily cadences resolve and decay on their own timescale. Each cadence's inference ceiling is ~1.5× its natural publish period (the same values as `halfLifeHours`), so a show at its nominal cadence (24h daily, 7d weekly) lands squarely in its own band. Do not resurrect the removed `freshnessHalfLifeDays` column.
...
`RecommendationEngine` caches the centroid+affinity result in `ThreadSafe<ScoringContext?>(nil)`. `Observatory.scoringContextInputsWithoutPartialSignals()` emits a `ScoringContextInputs` snapshot — small Sendable+Equatable struct (`ratedSignals`, `signalEmbeddings`, `embeddingCount`) so `removeDuplicates()` works without forcing Equatable on `Episode`/`UnsavedEpisode`, while deliberately excluding playback-path partial signals from the tracked region. Engine exposes a sync `func start()` matching the codebase convention (`startOnce.run { startObservingScoringContext() }`, mirroring `StateManager`/`CacheManager`/`WidgetSnapshotWriter`). The observation Task runs at `.utility` via `Container.shared.taskPriority()` so tests don't get starved by priority inheritance. **Public scoring methods don't await `start()`** — `recommendations(for:)` / `topRecommendations()` just read whatever the cache currently holds and return empty when it's nil. Lifecycle owners are responsible for hydration: AppLauncher calls `recommendationEngine.start()` during `prepareForForeground` so the cache is hot by the time any UI surface asks; tests call `engine.start()` after seeding their fixture and (when expecting non-empty results) poll `engine.topRecommendations()` via `Wait.forValue` until the cache lands. As of the listened-coverage work, rebuilds are debounced and re-read `Repo.allScoringContextInputs()` so GRDB observation emissions and onDeck session-boundary triggers collapse into one fresh snapshot.
...
### On-demand embedding processing — updated 2026-07-21

`EmbeddingProcessor` drains continuously while the app is active and keeps a `BGProcessingTask` pending while backgrounded only when embedding work may exist. The foreground observer still debounces content-change emissions before querying `episodesNeedingEmbeddings`; periodic one-hour submissions are gone. Background requests use the scheduler's on-demand mode with a one-minute earliest begin date, and draining an empty backlog cancels any successor request.

Demand is a persisted generation plus the last completed model/recipe version. Feed refreshes mark it after their database update commits and before the refresh task completes, then schedule the foreground drain or background request for the current lifecycle mode. Registration reconciles persisted demand with the database, and a model or recipe change forces one full verification pass so source hashes can be refreshed even when timestamps are current. The forced refresh also persists a pipeline-specific start time; each completed chunk advances its rows' verification dates beyond that boundary, so a task expiration or process restart resumes with the unfinished rows instead of restarting the library.

Clearing is race-safe: after processing, the processor re-queries the database and clears only if the backlog is empty and the persisted generation has not changed. A producer arriving during the final query therefore leaves demand pending. Cancellation, expiration, asset unavailability, query failure, and retryable embedding failure also preserve demand for the next background grant or foreground activation. No periodic timer or polling loop drives background scheduling.

Per-episode embedding failures persist in `episodeEmbeddingFailure`, keyed by the episode and podcast content timestamps plus model revision and recipe version. Matching failures remain eligible for three attempts; the third quarantines that exact input so it no longer sustains global demand or a successor background request. A successful compute or hash-stable verification clears prior failure state, while any content, model, or recipe change makes a quarantined episode eligible again. A forced pipeline refresh is marked complete after every row predating its start boundary has been verified, across as many background grants as needed, and any failed rows continue through the incremental query without restarting the library.

Asset readiness is now an `AsyncLatch<Void>` (`ContextualEmbedding.assetsLoaded`) instead of a plain `Bool` — the foreground task can `await embedding.assetsLoaded.wait()` cleanly without polling. The latch is a new general-purpose primitive in `PodHaven/Utility/AsyncLatch.swift`: monotonic one-shot signal carrying a `Value: Sendable` payload, cancellation-safe waits. `DownloadTask` was migrated at the same time (`beganLatch` and `finishedLatch` replacing two `[CheckedContinuation]` arrays plus `begun: Bool` / `result: DownloadResult?` state), which also fixed a latent leak where a cancelled `downloadBegan()` caller would never have its continuation resumed.

Files: `PodHaven/Utility/AsyncLatch.swift`, `PodHaven/Recommendations/Embeddings/EmbeddingProcessor.swift`, `PodHaven/Recommendations/Embeddings/EmbeddingWorkDemand.swift`, `PodHaven/Recommendations/Embeddings/ContextualEmbedding.swift`, `PodHaven/Feed/RefreshManager.swift`, `PodHaven/Database/Observatory.swift`, `PodHaven/Database/RecommendationRepo.swift`, `PodHaven/Database/Models/EpisodeEmbeddingFailure.swift`, `PodHaven/Database/Migrations/Migration_v69.swift`, `PodHaven/Utility/DownloadManager.swift`. Tests: `AsyncLatchTests`, `EmbeddingProcessorTests` (demand persistence, on-demand scheduling, lifecycle handoff, draining, bounded per-input retries, cancellation, idempotence, and observation retry), `EmbeddingRepoTests` (failure eligibility and quarantine), `V69MigrationTests`, `RefreshManagerTests` (changed versus unchanged feeds), and the `assetsLoaded` waiter in `ContextualEmbeddingTests`.

### Contextual model preparation and recovery

Asset presence, Apple's asset-request result, and successful model loading are separate states. `Embeddable` preserves `NLContextualEmbedding.AssetsResult`; only `.available` permits a post-request load. A successful load opens the readiness latch. A failed load can trigger one asset re-request and one load retry in the current foreground activation. This is an experimental recovery strategy; Apple does not document it as a repair for compilation failures.

After exhaustion, another attempt needs a later foreground activation and at least 60 seconds since the previous sequence's most recent asset request. The check uses the monotonic clock. An activation inside the cooldown consumes that activation's opportunity without scheduling a timer. Scoring calls, background grants, repeated active notifications, and thermal recovery cannot restart an exhausted attempt. A cold background grant can still attempt an initial on-disk load, without downloading.

The processor's foreground activation starts when it enters `.active` after launch or `.background`; passing through `.inactive` alone does not start another activation. Thermal suspension retains the activation. One request remains in flight across lifecycle transitions. Apple's API has no request cancellation handle, so its completion records the outcome, but further loading or recovery waits until foreground and thermal eligibility allow it. An unfinished sequence resumes without another initial request. Duplicate completion callbacks are ignored. Callbacks weakly capture their originating actor instead of resolving another instance from Factory.

Episode-work observation continues while a request is pending. Demand remains persisted, and readiness releases waiting processing and refreshes non-cacheable unsaved scores. Model preparation does not change the per-episode failure quarantine, stored vectors, or background scheduling cadence.

Load errors remain errors. Only the observed `NLNaturalLanguageErrorDomain` code 7 asset-download timeout text is classified as a warning at the request boundary. Other code 7 failures remain errors; localized or changed timeout text therefore falls back to error severity. Request diagnostics include the result's raw value, revision, asset presence, prior/current model preparation state, operation, attempt counts, and duration. `modelSession` is a random identifier for that model instance, not a device identifier.

A previously failed model emits one warning-level `event=contextualEmbeddingRecovered` record when it becomes usable. This reaches the existing Sentry structured-log threshold; ordinary successful loads stay at info. Correlate the recovery with failures using `modelSession`, then existing environment/release attributes. Each model instance can emit only one recovery record; failed sequences are bounded by activation and cooldown. The identifier does not span process restarts. Routine loads and installs are not all visible in Sentry, so absent recovery logs do not establish uninterrupted failure or prove a distribution-channel transition.

The September 11 investigation found 66 logs across four affected installations with no observed channel switch. The switching hypothesis remains inconclusive. A Mac probe loaded successfully and did not reproduce the failure. Physical-device compilation recovery and TestFlight/App Store switching remain unverified; evaluate actual recovery outcomes after deployment before claiming the asset re-request repairs the Apple-side defect. See [issue #659](https://github.com/jubishop/podhaven/issues/659) for evidence and acceptance details.
