...
- The observation closure inside `Observatory.scoringContextInputs()` builds a `ScoringContextInputs` directly from rated signals, signal embeddings, the embedding-row count, and freshness cadences — with `partialSignals: []`. It intentionally omits the `PartialSignal` fetch so `playbackCoverage` and `lastPlayedDate` stay out of the tracked region; per-tick `updatePlayback` writes therefore do not wake the observation. `PartialSignal` is fetched only inside `Repo.latestScoringContextInputs()`, which the engine calls on rebuild and which fills `partialSignals` with the real list.
