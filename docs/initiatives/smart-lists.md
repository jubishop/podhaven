...
struct SmartListFilter: Codable, Hashable, Sendable {
  ...
  enum TextOp: String, Codable, Sendable { case contains, doesNotContain, equals }
...
- **Episode text conditions** (`.episodeText`): GRDB column expressions, not raw SQL. `contains`/`doesNotContain` match whole words and word-prefixes through the external-content `episode_fts` mirror, **column-scoped** to the queried field — `ftsColumn(field).match(FTS5Pattern(matchingAllPrefixesIn: value))` inside an `EpisodeFTS.select(Column.rowID).contains(Episode.Columns.id)` subquery (each mirror's `rowid` is its source row's `id`, so the match resolves straight back). `equals` stays an exact whole-field match via `column.collating(.nocase) == value`. `doesNotContain` is `!matches` (`NOT IN` the FTS subquery), which is null-safe and keeps rows whose text is NULL — they never enter the matched set. A value with no tokenizable content yields no `FTS5Pattern`: `contains` then matches nothing (`false.sqlExpression`) and `doesNotContain` matches everything (`AppDB.noOp`). `startsWith` is removed (FTS5 has no fast "field begins with" form); `Migration_v59` rewrites any saved `startsWith` filter to `contains`.
- **Podcast text conditions** (`.podcastText`): `Observatory.listablePodcastEpisodes(filter:)` takes only a flat `SQLExpression`, so route through the parent podcast with a `…contains(Episode.Columns.podcastId)` subquery instead of a join. `episode.podcastId` is NOT NULL (FK with cascade delete) and the matched ids are primary keys, so `IN`/`NOT IN` membership is null-safe and no orphan branch is needed.
  - `contains`/`doesNotContain` match against the `podcast_fts` mirror (same column-scoped prefix match as the episode case), selecting `Column.rowID` (= podcast `id`) and matching it against `Episode.Columns.podcastId`. `doesNotContain` negates the membership (`!matches`) and stays null-safe for the same reason as the episode case.
  - `equals` routes through the `podcast` table itself — `Podcast.select(Podcast.Columns.id).filter(column.collating(.nocase) == value).contains(Episode.Columns.podcastId)` (GRDB auto-aliases the subquery's `podcast`, isolating it from the request's joined `podcast`).
