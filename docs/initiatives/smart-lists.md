...
**Why Phase 1 copies but does not delete the old sort-pref keys:** if Phase 1 reaches users before Phase 2, the old `EpisodesListView` still reads `EpisodesList-sortMethod-{title}`. So the v54 migration *copies* those prefs onto seeded rows but leaves the keys in place; Phase 2 — which removes the old reader — deletes them. Migrations are immutable, so the destructive cleanup belongs in Phase 2 code, not the migration.
...
3. Launch in simulator:
   ...
   - Upgrade install (from a v53 DB if possible): migration succeeds, all 10 lists present, seeded sort matches the pre-upgrade pref. After Phase 2 ships, confirm the 10 `EpisodesList-sortMethod-{title}` keys are gone.
