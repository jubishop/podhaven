...
The current condition set is **seven condition kinds**: episode text (title/description), episode transcript text, podcast text (title/description), episode state, effective-tag membership, **duration range**, and **publish-date age**. A tag condition matches episodes whose own episode row has that tag OR whose parent podcast has that tag. v65 migrates saved filters from the old split tag scopes into this single `tag` condition while preserving podcast title/description text conditions.
...
When a `Tag` is deleted, every Smart List filter that references it is **scrubbed in the same write transaction** (decode → drop the matching `.tag` conditions → re-encode → update). List cardinality is low, so this is cheaper than building an "unknown tag" placeholder state in the editor. The engine and editor remain **defensive** anyway — a tag ID that fails to resolve matches nothing rather than crashing — to cover cross-device / mid-delete races.
...
struct SmartListFilter: Codable, Hashable, Sendable {
  ...
  enum Condition: Codable, Hashable, Sendable {
    ...
    case podcastText(TextField, TextOp, String)  // podcast title / description
    case state(StateCondition)
    case tag(TagCondition)  // episode tag OR parent-podcast tag
    case duration(minSeconds: Int?, maxSeconds: Int?)   // inclusive; nil = open-ended
    case publishDate(PublishDateOp, days: Int)
  }

  // titleOrDescription matches both columns at once (whole virtual table).
  enum TextField: String, Codable, Sendable { case title, description, titleOrDescription }
  enum TextOp: String, Codable, Sendable { case contains, doesNotContain }
  ...
  enum StateCondition: String, Codable, Sendable {
    case isQueued, isUnqueued, isFinished, isUnfinished, isStarted, isUnstarted,
         isCached, isUncached, isSaved, isLoved, isLiked, isDisliked, isNotInterested,
         isRated, isUnrated, wasPreviouslyQueued, isTranscribed, isNotTranscribed
  }
...
`.tag` deliberately treats episode and podcast tag mappings as one effective tag set for each episode. The shared `TagCondition` payload keeps the four membership predicates defined once. `.episodeText` and `.podcastText` remain separate cases sharing the `TextField`/`TextOp` payloads. `.episodeTranscript` shares only `TextOp` because it has no title/description field selector.

Custom `init(from:)` / `encode(to:)` on `Condition` and `TagCondition` write a stable `kind` string for each payload variant (`"episodeText"`, `"episodeTranscript"`, `"podcastText"`, `"state"`, `"tag"`, `"duration"`, `"publishDate"`). Decoding an unknown `kind` throws, which surfaces as a thrown error from the repo/observation read paths — the migration's CHECK guards the JSON shape at write time, and every production write encodes from a typed value, so an undecodable row is unreachable without an app downgrade. v65 is the migration boundary for old `"episodeTag"` / `"podcastTag"` rows; the current decoder does not carry those legacy tag cases.

`SmartListFilter` also exposes a pure `func removingTag(_ id: Tag.ID) -> SmartListFilter` that strips every `.tag` condition whose `TagCondition` carries `id` (in the top group and every nested group, dropping a group emptied by the scrub), returning a value with the same combinators. Used by the scrub-on-delete path (§5).
...
- **State conditions** → existing constants: `isQueued`→`Episode.queued`, `isUnqueued`→`Episode.unqueued`, `isFinished`→`Episode.finished`, `isUnfinished`→`Episode.unfinished`, `isStarted`→`Episode.started`, `isUnstarted`→`Episode.unstarted`, `isCached`→`Episode.cached`, `isUncached`→`!Episode.cached`, `isSaved`→`Episode.savedInCache`, `isLoved`→`Episode.loved`, `isLiked`→`Episode.liked`, `isDisliked`→`Episode.disliked`, `isNotInterested`→`Episode.notInterested`, `isRated`→`Episode.rated`, `isUnrated`→`!Episode.rated`, `wasPreviouslyQueued`→`Episode.previouslyQueued`; `isTranscribed` / `isNotTranscribed` map to `episode.transcript IS NOT NULL` / `IS NULL`. (`isSaved`→`Episode.savedInCache` kept atomic since `saveInCache` isn't separately user-pickable in v1.)
- **Episode text conditions** (`.episodeText`): GRDB column expressions, not raw SQL. `contains`/`doesNotContain` match the value as a phrase — whole words, contiguous and in order, appearing anywhere in the field (no prefix matching) — through the external-content `episode_fts` mirror, scoped per `TextField`: `title`/`description` filter a single column, `titleOrDescription` matches the whole virtual table (both columns), and because an FTS5 phrase never spans columns that is exactly the phrase appearing in title or in description. Built via `ftsRowIDs(EpisodeFTS.self, field, FTS5Pattern(matchingPhrase: value))`, an `EpisodeFTS.select(Column.rowID).contains(Episode.Columns.id)` subquery (each mirror's `rowid` is its source row's `id`, so the match resolves straight back). `doesNotContain` is `!matches` (`NOT IN` the FTS subquery), which is null-safe and keeps rows whose text is NULL — they never enter the matched set. A value with no tokenizable content yields no `FTS5Pattern`: `contains` then matches nothing (`false.sqlExpression`) and `doesNotContain` matches everything (`AppDB.noOp`). Only `contains`/`doesNotContain` are offered. `startsWith` is removed (FTS5 has no fast "field begins with" form); `Migration_v59` rewrites any saved `startsWith` filter to `contains`. `equals` is likewise removed; `Migration_v60` rewrites any saved `equals` filter to `contains`.
- **Episode transcript conditions** (`.episodeTranscript`): the same phrase and null-safe inclusion/exclusion semantics as other text conditions, through `episode_transcript_fts`. Each indexed row flattens `segments[].text` in timed order with spaces, so a phrase may span adjacent segments while JSON keys, timestamps, and locale metadata never become searchable terms. The FTS rowid is the episode id.
- **Podcast text conditions** (`.podcastText`): same `TextField` / `TextOp` semantics as episode text, but through the external-content `podcast_fts` mirror. The subquery selects matching podcast rowids and checks membership against `Episode.Columns.podcastId`, so the expression remains valid when evaluated against either the unjoined episode request or the joined listable request.
...
Migration v72 adds the non-null Boolean `showUnreadBadge` column with an enabled default, preserving the previously unconditional badge for existing Smart Lists. New lists also default to enabled. The editor persists the preference with the list's other configuration, and `EpisodesView` renders the unread count only when it is enabled. Changing this display preference does not reset `lastSeenEpisodeId`; the existing watermark advances only when navigation returns from the Smart List to the main Episodes view.

When `showUnreadBadge` is enabled, an open `EpisodesListView` uses that same watermark to highlight every matching episode whose id is greater than `lastSeenEpisodeId`. New episode arrivals do not advance the watermark. Opening an episode, switching tabs, and app lifecycle transitions leave it unchanged; `markSeen` runs only when navigation returns to the Episodes hub. The navigation row renders the state as a higher-contrast, full-width accent-tinted background with clear vertical separation between highlighted rows, and exposes high-priority `Status: New` accessibility content without replacing its existing metadata value. Disabling unread badges suppresses both the tint and the accessibility status.
...
- The Episodes tab uses a plain `PathManager`; last-viewed navigation is intentionally not restored between launches. A smart list missing from the `sharedState.smartLists` mirror simply pops to the hub. Migration v71 removes the obsolete `navigationEpisodesTopDestination` and `navigationPodcastsTopDestination` defaults left by the retired path persistence.
...
- `SmartListEditorViewModel` — `@Observable @MainActor`; owns the editable title, filter, icon, artwork preference, and `showUnreadBadge`, plus `mode: .create | .edit(SmartList.ID)`. `save()` validates and persists the full configuration atomically through `SmartListRepo`. `delete()` is available in edit mode only.
- `SmartListEditorView` — sheet root; title and icon at top, native toggles for unread-badge visibility and artwork behavior, then one "Conditions" section containing the top group's segmented `.any/.all` picker, its condition rows and "Add Condition" button, every nested group rendered inline below them (so groups visibly participate in the same Any/All math), and a trailing "Add Group" button — adding a group is part of the same workflow as adding a condition, and each add button sits beside the rows it appends to rather than stacking identically-labeled buttons. Save/Cancel/Delete in the toolbar. Uses the canonical sheet pattern (`@DynamicInjected(\.sheet)` + `sheet(id:) { … }`, `Sheet.swift` / `PodHavenApp.swift`).
...
- `SmartListConditionRow` — a leading Kind picker (Episode Title / Episode Description / Episode Title or Description / Episode Transcription / Podcast Title / Podcast Description / Podcast Title or Description / State / Tag / Duration / Publish Date); the trailing controls swap by Kind:
  - **Text:** a `TextOp` picker + a text field.
  - **State:** a picker over the `StateCondition` cases with human labels.
  - **Tag:** a `TagCondition` picker (hasTag / doesNotHaveTag / hasAnyTag / hasNoTags) plus, for the first two, a tag picker reading `sharedState.tags`.
...
3. **Filter engine edge-case tests:** nullable-description `doesNotContain` matches null rows; one-level-nested `any`-inside-`all` parenthesizes correctly (fixture designed so wrong precedence yields a different set); `contains` matches whole words and contiguous in-order phrases (never prefixes or out-of-order tokens) and is scoped to the queried field; `titleOrDescription` matches the phrase within either column (an FTS5 phrase never spans columns, so it equals the OR of the title and description single-field matches, verified for a row whose words are split across the two); transcript phrases can span adjacent timed segments without matching title-only text; transcribed/not-transcribed states partition by transcript presence; `isUncached` is the complement of `isCached`; an untokenizable value matches nothing (and `doesNotContain` then matches everything). **Tag tests:** `hasNoTags` <-> `hasAnyTag` complementarity across episode and parent-podcast tag mappings; `hasTag` / `doesNotHaveTag` partition; a missing/unresolved `Tag.ID` makes `hasTag` match nothing. **Duration tests:** `minSeconds`/`maxSeconds` bounds are inclusive and open-ended on `nil`; `minSeconds: 0` includes unknown-duration (`0`) rows. **PubDate tests:** fixtures anchored to the current process time and kept clear of the SQLite `now` cutoff prove `withinLast` and `olderThan` partition around the query-time window.
4. **Codable round-trip** (`SmartListFilterCodableTests.swift`): the 10 default filters + a fixture with nested groups + fixtures for each condition kind (`episodeText`, `episodeTranscript`, `podcastText`, `state`, `tag`, `duration`, `pubDate`) and the new values within existing kinds (`titleOrDescription`, `isUncached`, `isTranscribed`, `isNotTranscribed`): encode → decode → encode → assert byte-equal. Decoding an unknown `Condition` `kind` throws.
...
3. Launch in simulator:
   ...
   - Build a tag filter for a tag used by both an episode and a podcast, Save → episodes from either source match; toggle the tag clause to a different tag and confirm the set changes.
