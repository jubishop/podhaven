...
When `showUnreadBadge` is enabled, an open `EpisodesListView` uses that same watermark to highlight every matching episode whose id is greater than `lastSeenEpisodeId`. New episode arrivals do not advance the watermark, and `markSeen` runs when the user leaves, so episodes arriving during an uninterrupted open session are highlighted too. The navigation row renders the state as a higher-contrast, full-width accent-tinted background with clear vertical separation between highlighted rows, and exposes high-priority `Status: New` accessibility content without replacing its existing metadata value. Disabling unread badges suppresses both the tint and the accessibility status.
