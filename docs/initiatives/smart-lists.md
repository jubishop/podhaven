...
- Tag conditions: subquery via raw `SQL`/`sqlExpression` — `Observatory.listablePodcastEpisodes(filter:order:limit:)` (`Observatory.swift:122–130`) only accepts a flat `SQLExpression` and offers no request-builder overload, so joins are not available. Use:
  ...
  - This is per-row but acceptable for v1; extending Observatory to take a request builder is a follow-up if perf bites.
...
- `SmartListConditionRow` — first picker chooses Kind (Title/Description/State/Tag); subsequent controls swap based on Kind. State picker covers the `StateCondition` cases with human labels. Tag picker reads from `sharedState.tags`. Each row has a leading "−" remove button (matches the screenshot reference shared in the brief).
...
3. **Filter engine edge-case tests**: nullable-description `doesNotContain` matches null rows; `hasNoTags` ↔ `hasAnyTag` complementarity; one-level-nested `any`-inside-`all` produces correctly parenthesized SQL (fixture designed so wrong precedence yields a different result set); `equals` and `startsWith` are case-insensitive; `startsWith` escapes `%` and `_`.
...
7. **VM live-update** (`EpisodesListViewModelTests.swift`): construct an `EpisodesListViewModel(smartList:)` against an in-memory DB; mutate the SmartList row's filter via `SmartListRepo.updateFilter`; assert the VM's `filter` and the displayed episode set update via `Wait.until`. Repeat for `updateSortMethod`.
