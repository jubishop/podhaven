...
`.tag` deliberately treats episode and podcast tag mappings as one effective tag set for each episode. The shared `TagCondition` payload keeps the four membership predicates defined once. `.episodeText` and `.podcastText` remain separate cases sharing the `TextField`/`TextOp` payloads.

Custom `init(from:)` / `encode(to:)` on `Condition` and `TagCondition` write a stable `kind` string for each payload variant (`"episodeText"`, `"podcastText"`, `"state"`, `"tag"`, `"duration"`, `"publishDate"`). Decoding an unknown `kind` throws, which surfaces as a thrown error from the repo/observation read paths — the migration's CHECK guards the JSON shape at write time, and every production write encodes from a typed value, so an undecodable row is unreachable without an app downgrade. v65 is the migration boundary for old `"episodeTag"` / `"podcastTag"` rows; the current decoder does not carry those legacy tag cases.
