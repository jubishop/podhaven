...
- **Podcast text conditions** (`.podcastText`): `Observatory.listablePodcastEpisodes(filter:)` takes only a flat `SQLExpression`, so route through the parent podcast with a `…contains(Episode.Columns.podcastId)` subquery instead of a join. `episode.podcastId` is NOT NULL (FK with cascade delete) and the matched ids are primary keys, so `IN`/`NOT IN` membership is null-safe and no orphan branch is needed.
  - `contains`/`doesNotContain` match against the `podcast_fts` mirror (same per-`TextField` prefix match as the episode case), selecting `Column.rowID` (= podcast `id`) and matching it against `Episode.Columns.podcastId`. `doesNotContain` negates the membership (`!matches`) and stays null-safe for the same reason as the episode case.
