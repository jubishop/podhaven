...
class AppStoreCommandTests(unittest.TestCase):
    ...
    def test_failed_local_tests_prevent_automatic_version_push_and_upload(self):
        ...
        self.assertIn("-skip-testing:PodHavenTests/SiriLargeCatalogTests", runs[1])
        self.assertIn("-only-testing:PodHavenTests/SiriLargeCatalogTests", runs[2])
        shutil.rmtree(self.repo / ".cache/test-all")
        result = self.run_command("--notes", "Fixes", TEST_BUILD_FAILURE="1")
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertEqual(self.gate_runs(), runs)
