...
def load_conversations(
    ...
    if terms:
        conversations = [
            item
            for item in conversations
            if all(term in item.search_text.lower() for term in terms)
        ]
...
def list_conversations(args: argparse.Namespace) -> int:
    conversations = load_conversations(args.projects_dir, args.cwd_contains, args.query)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def add_common_options(subparser: argparse.ArgumentParser) -> None:
        ...
        subparser.add_argument(
            "--query",
            help="Optional case-insensitive keyword query (space-separated terms).",
        )

    list_parser = subparsers.add_parser("list", help="List conversations")
    add_common_options(list_parser)
    list_parser.add_argument("--limit", type=int, default=15, help="Maximum rows to show")
    ...
    pick_parser.add_argument("--limit", type=int, default=15, help="Maximum rows to show")
