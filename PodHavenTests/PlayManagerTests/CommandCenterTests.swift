...
@MainActor struct CommandCenterTests {
  ...
  func mediaServicesResetWithEpisodeResetsFactoryScopesAndReregistersHandlers() async throws {
    ...
    // Capture initial factory instances
    ...
    // Verify factory scopes were reset (new instances created)
    let newCommandCenter = mpRemoteCommandCenter
    #expect(initialCommandCenter !== newCommandCenter)

    // Verify handlers are registered on the fresh command center
    #expect(newCommandCenter.play.isEnabled == true)
    #expect(newCommandCenter.pause.isEnabled == true)
    #expect(newCommandCenter.skipForward.isEnabled == true)
    #expect(newCommandCenter.skipBackward.isEnabled == true)

    // Verify commands are functional on the new instance
    newCommandCenter.firePlay()
  ...
  func mediaServicesResetWithNoEpisodeThenManualLoadRestoresControls() async throws {
    ...
    // Trigger media services reset with nothing on deck (the exact bug scenario:
    // handleItemStatusChange raced and cleared onDeck before the reset handler ran)
    notifier.continuation(for: AVAudioSession.mediaServicesWereResetNotification)
      .yield(Notification(name: AVAudioSession.mediaServicesWereResetNotification))
    try await PlayHelpers.waitForConfigureCallCount(callCount: initialCallCount + 1)

    // Verify factory scopes were reset
    let postResetCommandCenter = mpRemoteCommandCenter
    #expect(initialCommandCenter !== postResetCommandCenter)
    ...
    // Audio session should be reconfigured during performLoad
    let finalCallCount = await audioSession.configureCallCount
    #expect(finalCallCount >= initialCallCount + 2)

    // Verify commands are functional: play via command center, then skip forward
    postResetCommandCenter.firePlay()
    try await PlayHelpers.waitFor(.playing)

    postResetCommandCenter.fireSkipForward(TimeInterval.seconds(30))
    try await PlayHelpers.waitFor(CMTime.seconds(Double(30)))
  }
