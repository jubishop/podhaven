...
class ErrorTests {
  ...
  @Test("loggableMessage with caught wrapping NSError with underlying")
  func testLoggableMessageWithCaughtWrappingNSErrorWithUnderlying() {
    // When a ReadableError wraps an NSError that has NSUnderlyingErrorKey,
    // the underlying chain is shown via the caught error's loggable output
    let underlying = NSError(
      domain: "SystemDomain",
      code: 99,
