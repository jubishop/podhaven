...
@Suite("of v54 migration tests", .container)
class V54MigrationTests {
  ...
  // Seed at v53 with one podcast, one episode, and one embedding row so the
  // new covering index has data beneath it when v54 runs.
  private func populateAtV53() async throws {
      ...
      try db.execute(
        sql: """
          INSERT INTO podcast (
            ...
            700, 'https://example.com/v54.xml', 'V54 Podcast',
  ...
  @Test("v54 adds the covering embedding-needs index")
  func addsCoveringEmbeddingIndex() async throws {
    try await populateAtV53()
  ...
  @Test("v54 leaves existing embedding rows intact")
