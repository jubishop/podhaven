...
class V54MigrationTests {
  ...
  @Test("v54 creates, indexes, and seeds the smartList table")
  ...
  @Test("v54 sortMethod CHECK accepts recommendationScore and rejects unknown values")
  ...
  @Test("v54 filter CHECK enforces the SmartListFilter top-level shape")
  ...
  @Test("v54 copies UserDefaults sort prefs onto rows without deleting the keys")
  ...
  @Test("v54 leaves v53 data intact")
  func leavesV53DataIntact() async throws {
    try migrator.migrate(appDB.unsafeTestDB, upTo: "v53")
    try await appDB.unsafeTestDB.write { db in
      try db.execute(
        sql: """
