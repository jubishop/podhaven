...
class V60MigrationTests {
  ...
  // Seed at v59 with one podcast + one episode, before the transcript column.
  private func seedEpisodeAtV59() async throws {
      ...
      try db.execute(
        sql: """
          INSERT INTO podcast (
            ...
            700, 'https://example.com/v60.xml', 'V60 Podcast',
  ...
  @Test("v60 adds a nullable transcript column, NULL for existing episodes")
  func addsNullableTranscriptColumn() async throws {
    try await seedEpisodeAtV59()
  ...
  @Test("v60 stores timed-segment transcript JSON")
