...
actor ObservatoryTests {
  ...
  func testPodcastsWithEpisodeMetadataFeedURLsExisting() async throws {
    ...
    try await repo.insertSeries(
      unsavedPodcast1,
      unsavedEpisodes: [
        Create.unsavedEpisode(pubDate: 10.minutesAgo),
        Create.unsavedEpisode(pubDate: 20.minutesAgo),
      ]
    )
    try await repo.insertSeries(
      unsavedPodcast2,
      unsavedEpisodes: [Create.unsavedEpisode(pubDate: 5.minutesAgo)]
    )
    try await repo.insertSeries(
      unsavedPodcast3,
      unsavedEpisodes: [
    ...
    #expect(podcast1Metadata.mostRecentEpisodeDate!.approximatelyEquals(10.minutesAgo))

    let podcast2Metadata = podcastsWithMetadata.first { $0.podcast.feedURL == feedURL2 }!
    #expect(podcast2Metadata.episodeCount == 1)
    #expect(podcast2Metadata.mostRecentEpisodeDate!.approximatelyEquals(5.minutesAgo))
  }
  ...
  func testPodcastsWithEpisodeMetadataFeedURLsAsyncSequence() async throws {
    ...
    #expect(podcast1Current.mostRecentEpisodeDate!.approximatelyEquals(5.minutesAgo))
    ...
    #expect(podcast2Current.mostRecentEpisodeDate!.approximatelyEquals(10.minutesAgo))
    ...
    let newEpisode = try Create.unsavedEpisode(pubDate: 1.minutesAgo)
    ...
    #expect(podcast1Final.mostRecentEpisodeDate!.approximatelyEquals(1.minutesAgo))
  }
}
